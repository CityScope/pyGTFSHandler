# -*- coding: utf-8 -*-
"""Standalone pre-processing tool for stacking successive GTFS publications
from the same agency (e.g. one export per week) into a single merged GTFS
folder, *before* handing the result to `Feed`.

This is deliberately a separate, manual step rather than something `Feed`
does automatically when given multiple paths: `Feed(gtfs_dirs=[...])` treats
each path as an independent feed (namespacing every id by `_file_<n>` so
same-looking ids from different feeds never collide -- see
`io.read_csv_lazy`) and does not attempt to detect or merge genuinely
duplicate entities across them. If the caller knows several paths really are
the same agency's successive publications and wants routes/trips/stops/
service_ids reconciled across them into one coherent feed, they should run
that reconciliation here first (via `historic_stack`) and load the single
resulting folder through `Feed` -- not pass the raw paths straight to `Feed`.
"""
...
def stops(paths,new_path,aoi=None):
    schema_dict = gtfs_checker.get_df_schema_dict("stops.txt")
    df = io.read_csv_list([p / "stops.txt" for p in paths],schema_overrides=schema_dict)
    df = df.collect()
...
def routes(paths,new_path):
    schema_dict = gtfs_checker.get_df_schema_dict("routes.txt")
    df = io.read_csv_list([p / "routes.txt" for p in paths],schema_overrides=schema_dict)
    df = df.collect()
...
def trips_1(
    ...
    schema_dict = gtfs_checker.get_df_schema_dict("trips.txt")
    df = io.read_csv_list([p / "trips.txt" for p in paths],schema_overrides=schema_dict)
    df = df.collect()
    df = df.with_columns(
...
def stop_times(paths,new_path,trip_data=None,trip_ids_with_file_id=[],stop_ids_with_file_id=[],stop_ids=[]):
    schema_dict = gtfs_checker.get_df_schema_dict("stop_times.txt")
    df = io.read_csv_list([p / "stop_times.txt" for p in paths],schema_overrides=schema_dict)
    if len(stop_ids) > 0:
        ids_df = pl.LazyFrame({"stop_id": stop_ids, "isin_aoi":True})
        df = df.join(ids_df, on="stop_id", how="left")
        df = df.with_columns(pl.col("isin_aoi").fill_null(False))
...
def shapes(paths,new_path):
    schema_dict = gtfs_checker.get_df_schema_dict("shapes.txt")
    df = io.read_csv_list([p / "shapes.txt" for p in paths],schema_overrides=schema_dict)
...
def frequencies(paths,new_path,trip_ids_with_file_id=[],trip_ids=[],rename_trips=None):
    schema_dict = gtfs_checker.get_df_schema_dict("frequencies.txt")
    df = io.read_csv_list([p / "frequencies.txt" for p in paths],schema_overrides=schema_dict)
    df = df.collect()
    if len(trip_ids) > 0:
...
def calendar(paths,new_path,rename_services=None):
    schema_dict = gtfs_checker.get_df_schema_dict("calendar.txt")
    df = io.read_csv_list([p / "calendar.txt" for p in paths],schema_overrides=schema_dict)
    df = df.collect()
    df = df.with_columns(
        pl.concat_str([pl.col("service_id"),pl.lit("_"),pl.col("file_id")]).alias("service_id")
...
def calendar_dates(paths,new_path,rename_services=None):
    schema_dict = gtfs_checker.get_df_schema_dict("calendar_dates.txt")
    df = io.read_csv_list([p / "calendar_dates.txt" for p in paths],schema_overrides=schema_dict)
    df = df.collect()
    df = df.with_columns(
        pl.concat_str([pl.col("service_id"),pl.lit("_"),pl.col("file_id")]).alias("service_id")
    ).drop(["file_id","gtfs_name"])
...
def historic_stack(paths,new_path,aoi=None):
    ...
    for filename in filenames:
        df = io.read_csv_list([p / filename for p in paths]) 
        df = df.drop(["file_id","gtfs_name"]).unique()
