# -*- coding: utf-8 -*-
"""Service-quality/frequency-classification helper functions used by the
downstream analysis and plotting code (`plot_helpers.py`, notebooks/scripts)
-- e.g. bucketing headways into qualitative service-quality tiers.

These operate on already-computed `Feed` analysis output (from
`feed_analysis.py`/`feed_analysis_edges.py`), not on raw GTFS files, and are
plain numpy/pandas (not polars) since they work on small, already-aggregated
per-stop/per-edge tables rather than the full schedule.
"""
